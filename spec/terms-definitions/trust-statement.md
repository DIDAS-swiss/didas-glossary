...
~ A signed artefact, published under a [[ref: trust protocol]], that expresses trust or governance information about one or more actors or interactions, and that another actor can validate as an input to trust evaluation. A statement is issued or published by a governing or otherwise authoritative actor. It may concern a specific actor identifier and may be provisioned through the protocol by that actor, for example through issuer metadata or an authorization request. It is not a credential held by the subject, and being the subject or the carrier of a statement does not itself establish the resulting [[ref: trust marker]] or authorise the interaction: the evaluating actor validates the statement and derives the marker. What follows from a valid statement is determined by the trust protocol's evaluation rules and by the evaluating party's own policy.
