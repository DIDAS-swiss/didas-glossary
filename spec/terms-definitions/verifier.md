...
~ For the EID the limit is statutory, not only protocol-level. BGEID Art. 23 lets a verifier request the personal data the EID contains only where the check of identity, or of a partial aspect of it, is provided for in legislation, or where it is strictly necessary for the reliability of the transaction, in particular to prevent misuse and identity theft. A breach is entered in the trust register, visible to the holder during a transaction, and can mean exclusion from the register. The Act sets no equivalent test for other electronic credentials; what a verifier may ask for there is left to the [[ref: governance]] of the [[ref: trust domain]].
